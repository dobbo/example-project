...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "project-ear.ear");

        // Current directory is the root of the "project-web" project. Go up one level
        // and enter the "project-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../project-ejb/target/project-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "project-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/project-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "project-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
